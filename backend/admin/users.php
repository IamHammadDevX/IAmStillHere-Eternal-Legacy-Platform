...
try{
 ...
 if($_SERVER['REQUEST_METHOD']==='GET'){
  ...
  $s=$conn->prepare("SELECT id,username,email,full_name,role,status,created_at,last_login, (SELECT COALESCE(SUM(m.file_size),0) FROM memories m WHERE m.user_id=users.id AND m.status='active') + (SELECT COALESCE(SUM(pm.file_size),0) FROM post_media pm INNER JOIN posts p ON p.id=pm.post_id WHERE p.user_id=users.id AND p.status='active') AS memory_storage_bytes FROM users WHERE $where ORDER BY created_at DESC LIMIT :limit OFFSET :offset");foreach($p as $k=>$v)$s->bindValue(':'.$k,$v);$s->bindValue(':limit',$limit,PDO::PARAM_INT);$s->bindValue(':offset',$offset,PDO::PARAM_INT);$s->execute();
