...
function vault_storage_dir(): string { $path=getenv('VAULT_STORAGE_PATH') ?: dirname(dirname(BASE_PATH)) . DIRECTORY_SEPARATOR . 'iamalwayshere_vault_storage'; if(!is_dir($path)) @mkdir($path,0770,true); if(vault_path_is_public($path)) throw new RuntimeException('Vault storage path must be outside public web root.'); return $path; }
function vault_key_file(): string { $path=getenv('VAULT_KEY_PATH') ?: dirname(dirname(BASE_PATH)) . DIRECTORY_SEPARATOR . 'iamalwayshere_vault_keys' . DIRECTORY_SEPARATOR . 'master.key'; $dir=dirname($path); if(!is_dir($dir)) @mkdir($dir,0770,true); if(vault_path_is_public($dir)) throw new RuntimeException('Vault key path must be outside public web root and repository.'); if(!is_file($path)){ $key=base64_encode(random_bytes(32)); if(@file_put_contents($path,$key,LOCK_EX)===false) throw new RuntimeException('Vault key unavailable.'); @chmod($path,0600); } return $path; }
