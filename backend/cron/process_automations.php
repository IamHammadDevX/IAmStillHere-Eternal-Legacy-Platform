...
try{
    ...
    foreach($rules as $rule){
        ...
        foreach($all as $action){
            ...
            try{automation_worker_run_action($db,$rule,$action);$db->prepare("UPDATE automation_runs SET status='completed',completed_at=CURRENT_TIMESTAMP WHERE idempotency_key=:k")->execute(['k'=>$key]);}
