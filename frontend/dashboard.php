...
<head>
    ...
    <link rel="stylesheet" href="css/style.css?v=2026082702">
</head>
...
<body class="dashboard-page app-page">
    ...
    <main class="dashboard-shell"><div class="container py-4 py-lg-5">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active" id="memories-tab">
                <div class="card mb-3"><div class="card-body"><div class="d-flex flex-wrap gap-2 justify-content-between align-items-center"><div><strong class="feature-heading">All Memories <button type="button" class="feature-info" aria-label="About All Memories" data-tooltip="Create folders and subfolders, then upload and organize important photos, videos, Word files, Excel files, PDFs, and other memories. Select a folder to view only the memories stored inside it.">i</button></strong><div class="small text-muted mt-1">Organize photos, videos, documents, and other memories in folders.</div></div><div class="d-flex gap-2"><input id="folder-search" class="form-control form-control-sm" placeholder="Search folders"><button class="btn btn-outline-primary memory-folder-add" id="new-folder-button" type="button"><i class="bi bi-folder-plus" aria-hidden="true"></i><span>New folder</span></button></div></div><div id="memory-folder-breadcrumb" class="small text-muted mt-2">All memories</div><div id="memory-folders" class="d-flex flex-wrap gap-2 mt-2"></div></div></div>
            ...
            <div class="tab-pane fade" id="events-tab">
                ...
                <div class="card shadow-sm"><div class="card-body"><div class="d-flex justify-content-between align-items-start gap-3 mb-3"><div><h5 class="mb-1 feature-heading">Event Automations <button type="button" class="feature-info" aria-label="About Event Automations" data-tooltip="Schedule future-dated emails, in-app notifications, or wall posts to share birthday and anniversary wishes, blessings, gratitude, love, and other meaningful messages at the right time.">i</button></h5><p class="small text-muted mb-0">Schedule future emails, notifications, and wall posts for meaningful occasions.</p></div><button class="btn btn-primary btn-sm" type="button" onclick="openAutomationModal()">New automation</button></div><div id="automations-container"></div></div></div>
            </div>
            <div class="tab-pane fade" id="vault-tab">
                <div class="card vault-shell"><div class="card-body vault-shell-body">
                    <header class="vault-header"><div class="vault-title-group"><span class="vault-title-icon" aria-hidden="true"><i class="bi bi-shield-lock"></i></span><div><h5 class="feature-heading">Secure Vault <button type="button" class="feature-info" aria-label="About Secure Vault" data-tooltip="Keep school certificates, accounting records, personal files, and other important documents private and encrypted. You can grant access only to another trusted account and review access activity in the audit history.">i</button></h5><p>Store important documents privately, encrypted, and accessible only to people you trust.</p></div></div><div class="vault-header-actions"><button class="btn vault-refresh" id="vault-refresh-btn" type="button"><i class="bi bi-arrow-clockwise"></i><span>Refresh</span></button><button id="vault-upload-trigger" class="btn vault-primary" type="button"><i class="bi bi-cloud-arrow-up"></i><span>Add document</span></button></div></header>
                    <div id="vault-status" class="alert d-none" role="alert"></div>
                    <section id="vault-reauth-box" class="vault-lock-banner"><div class="vault-lock-copy"><i class="bi bi-lock-fill"></i><div><strong>Vault locked</strong><span>Confirm your password to unlock protected documents.</span></div></div><form id="vault-reauth-form" class="vault-unlock-form"><input id="vault-password" type="password" class="form-control" placeholder="Enter your password" autocomplete="current-password"><button class="btn vault-unlock-button" type="submit"><i class="bi bi-key"></i> Unlock vault</button></form></section>
                    <div class="vault-toolbar"><div id="vault-breadcrumb">Vault / All documents</div><div class="vault-encryption-note"><i class="bi bi-lock"></i> Encrypted documents · private access</div></div>
                    <section class="vault-upload-panel"><div class="vault-section-heading"><div><h6>Documents</h6><p>Upload a document to your secure vault.</p></div><span class="vault-security-pill"><i class="bi bi-shield-check"></i> Protected</span></div><form id="vault-upload-form" class="vault-upload-form"><input id="vault-file" type="file" class="form-control" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.rtf,.odt,.jpg,.jpeg,.png" required><button class="btn vault-primary" type="submit"><i class="bi bi-cloud-arrow-up"></i><span>Upload document</span></button></form><p class="vault-file-help">PDF, Office documents, text files, JPG and PNG.</p></section>
                    <div id="vault-document-list" class="row g-3 vault-document-grid"></div>
                    <details class="vault-access-details"><summary><span><i class="bi bi-chevron-down"></i> Access and history</span><small>Permissions and activity</small></summary><div class="vault-access-grid"><section class="vault-subpanel"><div class="vault-subpanel-icon"><i class="bi bi-person-lock"></i></div><h6>Legal counsel access</h6><p>Grant or revoke access for an authorized counsel user.</p><div class="input-group"><input id="vault-counsel-user-id" type="text" class="form-control" placeholder="Username or name"><button id="vault-grant" class="btn vault-grant" type="button">Grant</button><button id="vault-revoke" class="btn vault-revoke" type="button">Revoke</button></div><div id="vault-permission-list" class="vault-permission-list"></div></section><section class="vault-subpanel"><div class="vault-subpanel-icon"><i class="bi bi-clock-history"></i></div><h6>Audit history</h6><p>Recent access and document activity.</p><div id="vault-log-list" class="vault-log-list"></div></section></div><section class="vault-shared-panel"><div><h6><i class="bi bi-people"></i> Open a shared vault</h6><p>Only use this when counsel has authorized your account.</p></div><div class="vault-shared-row"><input id="vault-owner-id" type="number" min="1" class="form-control" placeholder="Vault owner ID"><span id="vault-current-owner-label"></span></div></section></details>
                </div></div>
                ...
                <div class="modal-body">
                    <form id="automationForm"><input type="hidden" id="automation-id">
                        <div class="row g-3">
                            ...
                            <div class="col-12"><label class="form-label">Actions</label><div class="d-flex flex-wrap gap-3 automation-action-options"><label><input type="checkbox" class="automation-action" value="notification" checked> In-app notification</label><label><input type="checkbox" class="automation-action" value="wall_post"> Scheduled wall post</label><label><input type="checkbox" class="automation-action" value="email"> Email/message</label></div></div>
                            <div class="col-12 automation-recipient-field" id="automation-notification-fields">
                                <div class="automation-recipient-panel">
                                    <label class="form-label" for="automation-recipient-search">Notification recipient <span class="text-danger">*</span></label>
                                    <p class="small text-muted mb-2">Search by name, username, email, or enter the exact User ID.</p>
                                    <div class="input-group"><input id="automation-recipient-search" class="form-control" type="search" minlength="2" autocomplete="off" placeholder="Search user or enter User ID"><button id="automation-recipient-search-btn" class="btn btn-outline-primary" type="button"><i class="bi bi-search"></i> Search</button></div>
                                    <input id="automation-recipient-user-id" type="hidden" value="">
                                    <div id="automation-selected-recipient" class="automation-selected-recipient d-none mt-2" aria-live="polite"></div>
                                    <div id="automation-recipient-results" class="list-group automation-recipient-results mt-2" aria-live="polite"></div>
                                </div>
                            </div>
                            <div class="col-12 automation-recipient-field d-none" id="automation-email-fields">
                                <div class="automation-recipient-panel"><div class="row g-2">
                                    <div class="col-md-7"><label class="form-label" for="automation-recipient-email">Recipient email <span class="text-danger">*</span></label><input id="automation-recipient-email" class="form-control" type="email" maxlength="254" autocomplete="email" placeholder="recipient@example.com"></div>
                                    <div class="col-md-5"><label class="form-label" for="automation-recipient-name">Recipient name <span class="text-muted small">(optional)</span></label><input id="automation-recipient-name" class="form-control" maxlength="255" placeholder="Recipient name"></div>
                                </div></div>
                            </div>
    ...
    <footer class="bg-dark text-light py-4 app-footer">
        <div class="container text-center">
            ...
            <!-- Copyright -->
            <p class="mb-0 small">&copy; <span id="current-year"></span> <strong>SV mobile teleshoppe pvt. ltd.</strong> All rights reserved.
            </p>
        </div>
    </footer>
...
<div class="modal fade" id="editFolderModal" tabindex="-1"><div class="modal-dialog modal-dialog-scrollable"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Edit folder</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><form id="editFolderForm"><input type="hidden" id="edit-folder-id"><label class="form-label">Name</label><input id="edit-folder-name" class="form-control mb-2" maxlength="150" required><label class="form-label">Description</label><textarea id="edit-folder-description" class="form-control mb-2"></textarea><label class="form-label">Parent folder</label><select id="edit-folder-parent" class="form-select mb-3"><option value="0">No parent</option></select><div id="edit-folder-privacy"></div><div id="edit-folder-error" class="small text-danger mt-2"></div></form></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" form="editFolderForm" id="edit-folder-save" class="btn btn-primary">Save changes</button></div></div></div></div>
    <script src="js/privacy.js"></script><script src="js/dashboard.js?v=2026082701"></script><script src="js/feature_tooltips.js?v=2026082601"></script>
</body>
