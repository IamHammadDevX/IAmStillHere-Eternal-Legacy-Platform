...
async function loadUsers(page=1){adminUsersPage=page;const q=encodeURIComponent(ael('admin-user-search')?.value||''),status=encodeURIComponent(ael('admin-user-status')?.value||'');const d=await adminJson(await fetch(`${ADMIN_API}/users.php?q=${q}&status=${status}&limit=10&page=${page}`,{cache:'no-store'}));const tb=ael('users-table-body');if(!d.success){tb.innerHTML='<tr><td colspan="10">Unable to load users.</td></tr>';return;}tb.innerHTML=(d.data.users||[]).map(u=>`<tr><td>${u.id}</td><td><strong>${esc(u.full_name||u.username)}</strong><div class="small text-muted">${esc(u.username)}</div></td><td>${esc(u.email)}</td><td><span class="badge bg-${u.role==='admin'?'danger':'primary'}">${esc(u.role)}</span></td><td><span class="badge bg-${u.status==='active'?'success':'secondary'}">${esc(u.status)}</span></td><td>${esc(u.last_login||'Never')}</td><td>${bytes(u.memory_storage_bytes)}</td><td><span class="admin-token-usage" title="AI Avatar: ${Number(u.ai_avatar_tokens||0).toLocaleString()} | Autobiography: ${Number(u.ai_autobiography_tokens||0).toLocaleString()} | AI Messages: ${Number(u.ai_message_tokens||0).toLocaleString()}">${Number(u.ai_tokens_used||0).toLocaleString()}</span></td><td>${esc((u.created_at||'').slice(0,10))}</td><td>${u.role!=='admin'?`<button class="btn btn-sm btn-outline-success me-1" onclick="setUserStatus(${u.id},'active')">Activate</button><button class="btn btn-sm btn-outline-warning" onclick="setUserStatus(${u.id},'suspended')">Suspend</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="10">No users.</td></tr>';adminPager('users-pagination',d.data.pagination.page,d.data.pagination.pages,loadUsers);}
