...
async function loadTimeline() {
    try {
        ...
        container.innerHTML = ''; const byDateAsc = (a, b) => String(a.milestone_date || '').localeCompare(String(b.milestone_date || '')) || Number(a.id) - Number(b.id); const all = (data.milestones || []).slice().sort(byDateAsc), children = new Map();
        ...
        children.forEach(list => list.sort(byDateAsc));
        const parents = all.filter(item => Number(item.parent_id) <= 0).sort(byDateAsc), canManage = loggedInUser && (loggedInUser.id == profileUserId || loggedInUser.role === 'admin');
        ...
        const card = (item, list = [], child = false) => `<div class="timeline-item ${child?'timeline-child-item':''}" id="milestone-${item.id}"><div class="timeline-marker"></div><div class="timeline-content timeline-story-card"><div class="timeline-story-heading"><div><h5>${escapeHtml(item.title||'Untitled')} ${item.category?`<span class="badge bg-info ms-2">${escapeHtml(item.category==='Progress'?'Update':item.category)}</span>`:''}</h5><small class="text-muted"><i class="bi bi-calendar"></i> ${escapeHtml(dateText(item.milestone_date))}</small></div>${canManage?`<button class="btn btn-sm btn-outline-danger" onclick="deleteMilestone(${item.id})" aria-label="Timeline actions"><i class="bi bi-three-dots-vertical"></i></button>`:''}</div><p class="text-muted mb-2">${escapeHtml(item.description||'')}</p><span class="badge bg-secondary privacy-badge">${escapeHtml(item.privacy_level||'public')}</span>${!child?`<button type="button" class="timeline-expand-btn" data-timeline-toggle="${item.id}" aria-expanded="false"><i class="bi bi-chevron-down"></i><span>${list.length?`${list.length} update${list.length===1?'':'s'}`:'Expand journey'}</span></button><div class="timeline-children d-none" data-timeline-children="${item.id}">${list.map(x=>card(x,[],true)).join('')}${canManage?`<button type="button" class="btn btn-sm btn-outline-primary timeline-add-child" data-parent-id="${item.id}"><i class="bi bi-plus-circle me-1"></i>Add update</button>`:''}</div>`:''}</div></div>`;
        ...
        container.querySelectorAll('.timeline-story-heading > .btn-outline-danger').forEach(deleteButton=>{const handler=deleteButton.getAttribute('onclick')||''; const id=(handler.match(/deleteMilestone\((\d+)\)/)||[])[1]; deleteButton.removeAttribute('onclick'); const menu=document.createElement('div'); menu.className='timeline-delete-menu d-none'; const trash=document.createElement('button'); trash.type='button'; trash.className='btn btn-sm btn-outline-danger'; trash.setAttribute('aria-label','Delete timeline item'); trash.innerHTML='<i class="bi bi-trash"></i>'; trash.addEventListener('click',()=>{menu.classList.add('d-none'); if(id) deleteMilestone(Number(id));}); menu.appendChild(trash); deleteButton.parentElement.classList.add('timeline-action-wrap'); deleteButton.parentElement.appendChild(menu); deleteButton.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();container.querySelectorAll('.timeline-delete-menu').forEach(x=>x.classList.add('d-none'));const open=menu.classList.contains('d-none'); menu.classList.toggle('d-none',!open);});});        container.querySelectorAll('[data-timeline-toggle]').forEach(button=>button.addEventListener('click',()=>{const box=container.querySelector(`[data-timeline-children="${button.dataset.timelineToggle}"]`),open=!box.classList.toggle('d-none');button.setAttribute('aria-expanded',String(open));button.querySelector('i').className=open?'bi bi-chevron-up':'bi bi-chevron-down';}));
