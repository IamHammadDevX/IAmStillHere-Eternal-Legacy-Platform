...
function journeyCoverMarkup(j, className = 'journey-cover') { if (!j?.cover_url) return ''; return j.cover_media_type === 'video' ? `<video class="${className}" controls preload="metadata"><source src="${escapeHtml(j.cover_url)}"></video>` : `<img class="${className}" src="${escapeHtml(j.cover_url)}" alt="${escapeHtml(j.title || 'Journey cover')}">`; }
...
function renderJourneys(rows) {
  ...
  rows.forEach(j => {
    const col = jEl('div', 'col-12 col-lg-6'); const card = jEl('div', 'card journey-card h-100'); const cover = journeyCoverMarkup(j, 'journey-card-cover'); if (cover) card.insertAdjacentHTML('beforeend', cover); const body = jEl('div', 'card-body d-flex flex-column');
...
async function openJourneyModal(journey = null) {
  ...
  document.getElementById('journey-cover-media').value = '';
...
async function saveJourney(e) {
  ...
  try {
    ...
    const cover = document.getElementById('journey-cover-media')?.files?.[0];
    if (cover) await uploadJourneyCover(savedId, cover);
    bootstrap.Modal.getInstance(document.getElementById('journeyModal')).hide(); loadJourneys();
  } catch (err) { error.textContent = err.message; }
}

async function uploadJourneyCover(journeyId, file) {
  const form = new FormData(); form.append('journey_id', journeyId); form.append('media', file); form.append('csrf_token', csrfToken || '');
  const response = await fetch(`${JOURNEYS_API}/upload_cover.php`, {method: 'POST', headers: {'X-CSRF-Token': csrfToken || ''}, body: form});
  const data = await response.json(); if (!data.success) throw new Error(data.message || 'Journey saved, but cover upload failed.');
}
...
async function openJourney(id) {
  ...
  const j=data.data.journey; body.innerHTML=''; const cover=journeyCoverMarkup(j, 'journey-detail-cover'); if(cover) body.insertAdjacentHTML('beforeend', cover); body.appendChild(jEl('h5','mb-1',j.title)); body.appendChild(jEl('div','small text-muted mb-2',jDateRange(j))); body.appendChild(jEl('p','',j.description||''));
  body.appendChild(jEl('div','small mb-3','Participants: '+(data.data.participants||[]).map(p=>p.full_name).join(', ')));
  const list=jEl('div','journey-timeline'); const items=data.data.items||[]; const media=items.filter(item=>item.item_type==='media'); const notes=items.filter(item=>item.item_type!=='media');
  if(media.length){ const gallery=jEl('div','journey-media-gallery mb-4'); media.forEach(item=>{const tile=jEl('article','journey-media-tile'); tile.innerHTML=item.media_mime?.startsWith('video/')?`<video controls preload="metadata"><source src="${escapeHtml(item.media_url)}"></video>`:`<img src="${escapeHtml(item.media_url)}" alt="${escapeHtml(item.title)}">`; tile.appendChild(jEl('strong','small d-block mt-2',item.title)); tile.appendChild(jEl('div','small text-muted',`${item.contributor_name} - ${item.item_date || 'No date'}${item.status==='pending'?' - awaiting approval':''}`)); if(item.description)tile.appendChild(jEl('p','small mb-0',item.description)); if(j.can_manage && item.status==='pending'){const ap=jEl('button','btn btn-success btn-sm mt-2 me-1','Approve');ap.onclick=()=>reviewJourneyItem(item.id,'approve');const rj=jEl('button','btn btn-outline-danger btn-sm mt-2','Reject');rj.onclick=()=>reviewJourneyItem(item.id,'reject');tile.append(ap,rj);} gallery.appendChild(tile);}); body.appendChild(gallery); }
  if(!items.length) list.appendChild(jEl('div','text-muted','No shared memories or media have been added yet.'));
  notes.forEach(item=>{ const row=jEl('div','border-start ps-3 mb-3'); row.appendChild(jEl('div','fw-semibold',item.title)); row.appendChild(jEl('div','small text-muted',`${item.item_date || 'No date'} - ${item.item_type} - ${item.contributor_name} - ${item.status}`)); if(item.description)row.appendChild(jEl('div','small',item.description)); if(j.can_manage && item.status==='pending'){ const ap=jEl('button','btn btn-success btn-sm me-2 mt-1','Approve'); ap.onclick=()=>reviewJourneyItem(item.id,'approve'); const rj=jEl('button','btn btn-outline-danger btn-sm mt-1','Reject'); rj.onclick=()=>reviewJourneyItem(item.id,'reject'); row.append(ap,rj); } list.appendChild(row); }); body.appendChild(list); wrap.scrollIntoView({behavior:'smooth',block:'start'});
}
...
async function openJourneyItem(id){ activeJourneyId=id; document.getElementById('journey-item-title').value=''; document.getElementById('journey-item-description').value=''; document.getElementById('journey-item-date').value=''; document.getElementById('journey-item-media').value=''; const sel=document.getElementById('journey-item-select'); sel.innerHTML='<option value="event:0">New event note</option>'; const res=await fetch(`${JOURNEYS_API}/content.php`); const data=await res.json(); (data.data?.items||[]).forEach(item=>{const o=document.createElement('option'); o.value=`${item.item_type}:${item.id}`; o.textContent=`${item.item_type}: ${item.title}`; sel.appendChild(o);}); bootstrap.Modal.getOrCreateInstance(document.getElementById('journeyItemModal')).show(); }
async function saveJourneyItem(){ const error=document.getElementById('journey-item-error'); const save=document.getElementById('journey-item-save'); error.textContent=''; const media=document.getElementById('journey-item-media')?.files?.[0]; const [type,id]=document.getElementById('journey-item-select').value.split(':'); const title=document.getElementById('journey-item-title').value.trim(); const description=document.getElementById('journey-item-description').value; const itemDate=document.getElementById('journey-item-date').value; save.disabled=true; save.textContent=media?'Uploading...':'Adding...'; try{let data;if(media){const form=new FormData();form.append('journey_id',activeJourneyId);form.append('title',title);form.append('description',description);form.append('item_date',itemDate);form.append('media',media);form.append('csrf_token',csrfToken||'');const res=await fetch(`${JOURNEYS_API}/upload_media.php`,{method:'POST',headers:{'X-CSRF-Token':csrfToken||''},body:form});data=await res.json();}else{const payload={journey_id:activeJourneyId,item_type:type,source_id:Number(id),title,description,item_date:itemDate,csrf_token:csrfToken};const res=await fetch(`${JOURNEYS_API}/add_item.php`,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken},body:JSON.stringify(payload)});data=await res.json();}if(!data.success)throw new Error(data.message||'Unable to add item.'); bootstrap.Modal.getInstance(document.getElementById('journeyItemModal')).hide(); openJourney(activeJourneyId);}catch(e){error.textContent=e.message;}finally{save.disabled=false;save.textContent='Add item';} }
