...
async function loadTimeline(page = timelinePage) {
    ...
    try {
        ...
        container.innerHTML = ''; const byDateAsc = (a, b) => String(a.milestone_date || '').localeCompare(String(b.milestone_date || '')) || Number(a.id) - Number(b.id); milestoneCache = (data.milestones || []).slice().sort(byDateAsc);
        const all = milestoneCache, children = new Map(); all.filter(x=>Number(x.parent_id)>0).forEach(x=>{if(!children.has(String(x.parent_id)))children.set(String(x.parent_id),[]);children.get(String(x.parent_id)).push(x);});
        children.forEach(list=>list.sort(byDateAsc));
        const parents = all.filter(x=>Number(x.parent_id)<=0).sort(byDateAsc), canManage = loggedInUser && (loggedInUser.id == currentUserId || loggedInUser.role === 'admin');
        const card = (m,list=[],child=false) => `<div class="timeline-item ${child?'timeline-child-item':''}"><div class="timeline-marker"></div><div class="timeline-content timeline-story-card"><div class="timeline-story-heading"><div><h5>${escapeHtml(m.title||'Untitled')} ${m.category?`<span class="badge bg-info ms-2">${escapeHtml(m.category==='Progress'?'Update':m.category)}</span>`:''}</h5><small class="text-muted"><i class="bi bi-calendar"></i> ${escapeHtml(((v)=>{const [y,mo,d]=String(v||'').slice(0,10).split('-').map(Number);return y&&mo&&d?new Date(y,mo-1,d).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}):''})(m.milestone_date))}</small></div></div><p class="text-muted mb-2">${escapeHtml(m.description||'')}</p><span class="badge bg-secondary privacy-badge">${escapeHtml(m.privacy_level||'public')}</span>${!child?`<button type="button" class="timeline-expand-btn" data-dashboard-toggle="${m.id}" aria-expanded="false"><i class="bi bi-chevron-down"></i><span>${list.length?`${list.length} update${list.length===1?'':'s'}`:'Expand journey'}</span></button><div class="timeline-children d-none" data-dashboard-children="${m.id}">${list.map(x=>card(x,[],true)).join('')}${canManage?`<button type="button" class="btn btn-sm btn-outline-primary timeline-add-child" data-dashboard-parent="${m.id}"><i class="bi bi-plus-circle me-1"></i>Add update</button>`:''}</div>`:''}</div></div>`;
